...
load_gltf_mesh :: proc(
	...
	if prim.type != .triangles {
		log_warnf("First primitive is not triangles in %s", string(path))
		return {}, {}, false
	}
	...
	if position_acc == nil || position_acc.count == 0 {
		log_warnf("Missing position accessor in %s", string(path))
		return {}, {}, false
	}
	...
	vertices = make([]Mesh_Vertex, vertex_count, context.temp_allocator)
	...
	if prim.indices != nil && prim.indices.count > 0 {
		...
		indices = make([]u32, vertex_count, context.temp_allocator)
		for i in 0 ..< vertex_count {
			indices[i] = u32(i)
		}
	}
